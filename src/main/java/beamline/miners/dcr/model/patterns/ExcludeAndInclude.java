...
public class ExcludeAndInclude implements RelationPattern {
	...
	public void populateConstraint(UnionRelationSet unionRelationSet) {
		...
		for (Pair<String, String> exclude : excludeSet) {
			unionRelationSet
					.addDcrRelation(Triple.of(exclude.getLeft(), exclude.getRight(), DcrModel.RELATION.EXCLUDE));

		}
		for (Pair<String, String> include : includeSet) {
			unionRelationSet
					.addDcrRelation(Triple.of(include.getLeft(), include.getRight(), DcrModel.RELATION.INCLUDE));
	...
	private void precedence() {
		Set<Triple<String, String, DcrModel.RELATION>> dcrSequences = unionRelationSet
				.getDcrRelationWithConstraint(DcrModel.RELATION.SEQUENCE);
	...
	private void dfs(int sourceIndex, int sinkIndex, Set<String> inBetween, BitSet[] transitiveClosure,
			BitSet visited) {

		for (int k = 0; k < activityList.size(); k++) {
			if (k != sourceIndex & k != sinkIndex & dfgAdjacencyMatrix[sourceIndex].get(k)
					& transitiveClosure[k].get(sinkIndex) & !visited.get(k)) {
